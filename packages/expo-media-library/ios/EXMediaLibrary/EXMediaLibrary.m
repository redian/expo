...
@property (nonatomic, strong) NSMutableSet *assetsSaveDelegates;
...
{
  if (self = [super init]) {
    _assetsSaveDelegates = [NSMutableSet new];
  }
  return self;
}
...
UM_EXPORT_METHOD_AS(saveToAssetsAsync,
                    saveToAssetsAsync:(nonnull NSString *)localUri
                    resolve:(UMPromiseResolveBlock)resolve
                    reject:(UMPromiseRejectBlock)reject)
{
  ...
  NSURL *assetUrl = [self.class _normalizeAssetURLFromUri:localUri];
  UM_WEAKIFY(self)
  __block EXAssetsSaveDelegate *delegate = [EXAssetsSaveDelegate new];
  [_assetsSaveDelegates addObject:delegate];
  EXAssetsSaveCallback callback = ^(id asset, NSError *error){
    UM_STRONGIFY(self)
    [self.assetsSaveDelegates removeObject:delegate];
    if (error) {
